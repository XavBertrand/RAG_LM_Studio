...
def limit_tokens(texts, max_tokens=500):
    limited_texts = []
    for text in texts:
        tokens = tokenizer.tokenize(text)
        if len(tokens) > max_tokens:
            tokens = tokens[:max_tokens]
        limited_text = tokenizer.convert_tokens_to_string(tokens)
...
def embed(texts, embd):
    ...
    # DEBUG
    # with open(r"C:\Users\bertr\PycharmProjects\TestRAG\simple-rag-lmstudio-main\texts_debug.pkl", 'rb') as f:
    #     texts = pickle.load(f)

    # Truncate texts. Do not exceed 500 tokens
    texts_truncated = limit_tokens(texts, max_tokens=500)

    # text_embeddings = embd.embed_documents(texts)
    text_embeddings = embd.embed_documents(texts_truncated)
    text_embeddings_np = np.array(text_embeddings)
    return text_embeddings_np


def embed_cluster_texts(texts, embedding):
    """
...
def embed_cluster_summarize_texts(
    texts: List[str], level: int, embedding=None, llm_raptor=None
    ...
    df_clusters = embed_cluster_texts(texts, embedding)
    ...
    # Summarization
    template_EN = """Here is a sub-set of LangChain Expression Language doc. 
...
def recursive_embed_cluster_summarize(
    texts: List[str], level: int = 1, n_levels: int = 3, embedding=None, llm_raptor=None
    ...
    df_clusters, df_summary = embed_cluster_summarize_texts(
        texts, level, embedding, llm_raptor
    )
    ...
    if level < n_levels and unique_clusters > 1:
        ...
        next_level_results = recursive_embed_cluster_summarize(
            new_texts, level + 1, n_levels, embedding, llm_raptor
        )
